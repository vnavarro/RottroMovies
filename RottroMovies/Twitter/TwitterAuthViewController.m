...
- (void)requestTokenTicket:(OAServiceTicket *)ticket didFinishWithData:(NSData *)data {
  if (ticket.didSucceed) {
    ...
    self.request_url = [NSURL URLWithString:[NSString stringWithFormat:TWITTER_AUTHORIZE_URL,[TwitterAccessor sharedAccessor].request_token.key,@"http://www.vnavarro.com.br"]];
...
- (void)viewDidLoad
{
  ...
  //[twitter_accessor verifyCredentials];
  //[webview_auth loadHTMLString:twitter_accessor.test baseURL:  [NSURL URLWithString:@"www.twitter.com"]];
  [[TwitterAccessor sharedAccessor] verifyCredentialsWithDelegate:self];
  //if(!twitter_accessor.valid_credentials)
    //[twitter_accessor requestTokenWithDelegate:self]; 
}
