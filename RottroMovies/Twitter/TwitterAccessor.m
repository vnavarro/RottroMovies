...
-(void)retweetFinished:(OAServiceTicket *)ticket didFinishWithData:(NSData *)data{
  [self operationFinishedAlert:ticket.didSucceed withSuccessMessage:@"Retweeted!" andFailMessage:@"Oops something went wrong..."];
}
-(void)bookmarkFinished:(OAServiceTicket *)ticket didFinishWithData:(NSData *)data{
  [self operationFinishedAlert:ticket.didSucceed withSuccessMessage:@"Tweet favorited!" andFailMessage:@"Oops something went wrong..."];
}
...
-(void)getHomeTimelineTo:(id)delegate andFinishedSelector:(SEL)selector{
  NSString *home_timeline_url = [NSString stringWithFormat:TWITTER_HOME_TIMELINE_URL,@"?count=10"];
  [self makeRequestWithUrl:home_timeline_url andHttpMethod:@"GET" andDelegate:delegate withFinishedSelector:selector];
}

-(void)doRetweet:(NSString *)tweet_id{
  NSString *retweet = [NSString stringWithFormat:TWITTER_RETWEET_URL,tweet_id];
  [self makeRequestWithUrl:retweet andHttpMethod:@"POST" andDelegate:self withFinishedSelector:@selector(retweetFinished:didFinishWithData:)];
}

-(void)doBookmark:(NSString *)tweet_id{
  NSString *retweet = [NSString stringWithFormat:TWITTER_BOOKMARK_TIMELINE_URL,
                       [NSString stringWithFormat:@"/create/%@",tweet_id]];
  [self makeRequestWithUrl:retweet andHttpMethod:@"POST" andDelegate:self withFinishedSelector:@selector(bookmarkFinished:didFinishWithData:)];
}
...
-(void)doDM:(NSString *)status toUser:(NSString *)user{  
  [self sendDM:status toUser:user andDelegate:self withFinishedSelector:@selector(updateFinished:didFinishWithData:)];
}

-(void)getRetweetsTimeline:(id)delegate andFinishedSelector:(SEL)selector{
  NSString *retweets = [NSString stringWithFormat:TWITTER_RETWEET_TIMELINE_URL,@"?count=10"];
  [self makeRequestWithUrl:retweets andHttpMethod:@"GET" andDelegate:delegate withFinishedSelector:selector];
}

-(void)getMentionsTimeline:(id)delegate andFinishedSelector:(SEL)selector{
  NSString *mentions = [NSString stringWithFormat:TWITTER_MENTION_TIMELINE_URL,@"?count=10"];
  [self makeRequestWithUrl:mentions andHttpMethod:@"GET" andDelegate:delegate withFinishedSelector:selector];
}

-(void)getDMsTimeline:(id)delegate andFinishedSelector:(SEL)selector{
  NSString *dms = [NSString stringWithFormat:TWITTER_DM_TIMELINE_URL,@"",@"?count=10"];
  [self makeRequestWithUrl:dms andHttpMethod:@"GET" andDelegate:delegate withFinishedSelector:selector];
  dms = [NSString stringWithFormat:TWITTER_DM_TIMELINE_URL,@"/sent",@"?count=10"];
  [self makeRequestWithUrl:dms andHttpMethod:@"GET" andDelegate:delegate withFinishedSelector:selector];
}

-(void)getBookmarks:(id)delegate andFinishedSelector:(SEL)selector{
  NSString *bookmarks = [NSString stringWithFormat:TWITTER_BOOKMARK_TIMELINE_URL,@""];
  [self makeRequestWithUrl:bookmarks andHttpMethod:@"GET" andDelegate:delegate withFinishedSelector:selector];  
}
...
-(void)verifyCredentialsTicket:(OAServiceTicket *)ticket didFinishWithData:(NSData *)data{
  if(ticket.didSucceed) {
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:CHANGE_TWITTER_BUTTON object:nil];
...
- (void)sendDM:(NSString *)status toUser:(NSString *)user andDelegate:(id)delegate withFinishedSelector:(SEL)selector
{
  OAConsumer *consumer = [[[OAConsumer alloc] initWithKey:TWITTER_KEY
	...
	OAMutableURLRequest *oRequest = [[OAMutableURLRequest alloc] initWithURL:[NSURL URLWithString:TWITTER_DM_URL]
                                                                  consumer:consumer
	...
	OARequestParameter *statusParam = [[[OARequestParameter alloc] initWithName:@"text" value:status]autorelease];
  OARequestParameter *userParam = [[[OARequestParameter alloc] initWithName:@"screen_name" value:user]autorelease];
	NSArray *params = [NSArray arrayWithObjects:statusParam,userParam, nil];
